...
Contract: `docs/API.md` in the repo root (the backend follows it; `by_billy/docs/API.md` is an older copy). The TypeScript types in `src/lib/types.ts` match it one-to-one; all requests go through `src/lib/api.ts`.
