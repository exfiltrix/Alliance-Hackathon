...
Port `reference/muhr_poc.py` (already tested: finds a 1-pixel change, rejects forged ledger, ~2 ms per 512×512 image).

1. Read pixels: `pydicom.dcmread(f).pixel_array` or `np.array(Image.open(f))` (convert to grayscale, keep dtype).
2. Image ID (`uid`): DICOM `SOPInstanceUID`; for PNG generate a UUID and write it to a `muhr_uid` tEXt chunk.
