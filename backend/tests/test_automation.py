...
def tampered_copy(client, sealed_meta) -> bytes:
    sealed = client.get(sealed_meta["download_url"]).content
...
def test_batch_upload_ranks_by_urgency(client, device):
    ...
    good = client.get(meta["download_url"]).content
    files = [
        ("files", ("good.png", good)),
        ("files", ("unsigned.png", xray_png(seed=2))),
        ("files", ("tampered.png", tampered_copy(client, meta))),
