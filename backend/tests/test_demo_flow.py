...
FORGED_REASONS = {"ledger_entry_modified", "bad_signature", "device_revoked", "unknown_device", "blockchain_mismatch"}
...
def test_demo_flow(client, device, chain, tmp_path):
    ...
    assert "clean" in out
    assert client.get("/api/ledger/check").json()["ok"]  # our own ledger check is fooled...
    r = verify(fake)
    assert (r["status"], r["reason"]) == ("forged", "blockchain_mismatch")  # ...the chain is not
